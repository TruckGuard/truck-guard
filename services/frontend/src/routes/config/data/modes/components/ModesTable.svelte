<script lang="ts">
    ...
    let { modes, flex = false, currentUser = null, onEdit, onDelete } = $props<{
        modes: any[];
        flex?: boolean;
        currentUser?: any;
...
{#snippet row(mode: any)}
    ...
    <Table.Cell class="px-3 py-0 font-medium text-sm">{mode.name}</Table.Cell>
    <Table.Cell class="px-3 py-0 text-xs text-muted-foreground">{mode.description || "—"}</Table.Cell>
    <Table.Cell class="px-3 py-0 text-right">
        <div class="flex items-center justify-end gap-0.5">
            {#if can(currentUser, "update:data")}
                <Button variant="ghost" size="icon" class="size-7" onclick={() => onEdit(mode)} title="Редагувати">
                    <Pencil class="h-3.5 w-3.5" />
                </Button>
            {/if}
            {#if can(currentUser, "delete:data")}
                <Button variant="ghost" size="icon" class="size-7 text-destructive hover:bg-destructive/8 hover:text-destructive" onclick={() => onDelete(mode)} title="Видалити">
...
<DataTable columns={4} items={modes} {flex} headerSnippet={header} rowSnippet={row} emptyStateIcon={Database} />
