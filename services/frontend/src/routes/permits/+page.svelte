<script lang="ts">
  ...
  function refresh() {
    loading = true;
    goto(page.url.pathname, { invalidateAll: true }).then(
      () => (loading = false),
    );
  }
...
>
  ...
  <div class="flex h-full gap-6 overflow-hidden">
    <!-- Зліва: Стрічка подій -->
    <div
      class="w-1/3 flex flex-col bg-card border rounded-xl shadow-sm overflow-hidden shrink-0"
    >
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3">
        {#if sortedEvents.length === 0}
          <div
            class="h-32 flex items-center justify-center text-muted-foreground italic"
          >
          ...
          {#each sortedEvents as event}
            ...
            >
              ...
              <div class="flex items-center justify-between">
                {#if event._type === "plate"}
                  <div
                    class="inline-flex items-center border border-border rounded bg-white dark:bg-slate-950 px-2 shadow-sm"
                  >
                    <span
                      class="font-bold text-slate-900 dark:text-slate-50 tracking-widest font-mono uppercase text-sm"
                    >
                      {event.plate}
                    </span>
                  </div>
                {:else}
                  <div
                    class="font-mono text-lg font-bold text-blue-600 dark:text-blue-400"
                  >
                    {event.weight}
                ...
                <div
                  class="opacity-0 group-hover:opacity-100 transition-opacity flex items-center text-xs font-medium text-indigo-600 bg-indigo-50 dark:bg-indigo-950 px-2 py-1 rounded-full"
                >
    ...
    <div
      class="flex-1 flex flex-col bg-card border rounded-xl shadow-sm overflow-hidden min-w-0"
    >
      <div class="p-4 border-b bg-muted/40 flex items-center gap-2">
            ...
            {:else}
              {#each activePermits as permit}
                <Table.Row
                  ...
                  onclick={() => goto(`/permits/${permit.ID}`)}
                >
                  ...
                  <Table.Cell>
                    <div class="flex flex-col gap-1 text-sm">
                      {#if permit.vehicle_type}
                        <span
                          class="inline-flex w-fit px-1.5 py-0.5 rounded text-xs font-medium"
                          style="background-color: {permit.vehicle_type
                            .color}20; color: {permit.vehicle_type.color}"
                        >
                  ...
                  <Table.Cell class="text-right">
                    <div
                      class="inline-flex items-center gap-1.5 text-emerald-600 bg-emerald-50 dark:bg-emerald-950/30 px-2 py-1 rounded-full text-xs font-semibold"
                    >
